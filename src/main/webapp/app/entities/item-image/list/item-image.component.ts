...
export class ItemImageComponent implements OnInit {
  ...
  loadAll(): void {
    ...
    this.itemImageService
      ...
      .subscribe(
        (res: HttpResponse<IItemImage[]>) => {
          this.isLoading = false;
          this.paginateItemImages(res.body, res.headers);
        },
        () => {
          this.isLoading = false;
        }
  ...
  trackId(index: number, item: IItemImage): number {
    return item.id!;
  }
  ...
  protected paginateItemImages(data: IItemImage[] | null, headers: HttpHeaders): void {
    this.links = this.parseLinks.parse(headers.get('link') ?? '');
    if (data) {
      for (const d of data) {
        this.itemImages.push(d);
      }
    }
  }
}
